...
`Filter α` は，`α` の部分集合のうち「十分大きい」「十分近い」とみなす集合を集めたものです．
...
* `s ∈ l`: 集合 `s` はフィルター `l` の意味で十分大きい．
* `∀ᶠ x in l, p x`: `p x` が成り立つような `x` 全体の集合が，
  フィルター `l` の意味で十分大きい．
  定義上は `{x | p x} ∈ l` という主張です．
* `Tendsto f l m`: `x` が `l` に沿って動くとき，`f x` が `m` に沿って動く．
...
「`m` の意味で十分な集合の逆像は，`l` の意味で十分な集合である」と読むのがよいです．
