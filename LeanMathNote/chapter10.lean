...
| 段階 | 実体 | 見方 |
| --- | --- | --- |
| 文字列としてのソースコード | `.lean` ファイルや標準入力から渡される Unicode 文字列です．まだ Lean の構文木ではありません． | エディタで開く，`cat LeanMathNote/chapter10.lean` で見る，`lake env lean --stdin` に文字列を渡す． |
...
この章のように `LeanMathNote/chapter10.lean` を単体で `lake env lean` した場合は，
...
lake env lean --profile LeanMathNote/chapter10.lean
lean --print-prefix
lean --print-libdir
...
LeanMathNote/chapter10.lean   module LeanMathNote.gpt.chapter10
```
