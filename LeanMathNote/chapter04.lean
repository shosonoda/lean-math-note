...
* 選択: 存在から証人を選ぶ操作を可能にし，古典数学を扱いやすくする．
...
`Classical.choice` は非空な型から要素を選ぶ原理で，存在命題から具体的な証人を取り出す `Classical.choose` の基礎になっています．
このような定義は一般に計算できないので，`noncomputable` として宣言します．
`Classical.choice` の入力は `Nonempty α` であり，`∃ x, P x` から証人を取り出すときは `Classical.choose` と `Classical.choose_spec` を使う，という使い分けを押さえておくとよいです．
-/
...
5. `Classical.choose` と `Classical.choose_spec` を使って，存在命題から証人を取り出してください．
