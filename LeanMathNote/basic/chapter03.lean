...
ここでいう tactic は，Lean のカーネルが直接持っている推論規則そのものではありません．
tactic は，elaboration の途中で実行され，ゴールを操作しながら証明項を構成するためのプログラムです．
完成した証明項は，最後に Lean のカーネルによって型検査されます．
この章では tactic の使い方に集中し，elaboration やカーネルの詳しい仕組みは Chapter 06 で扱います．
...
ただしどちらの場合も，最終的には Lean のカーネルが証明項を型検査している点は同じです．
-/
...
tactic は証明を魔法のように作っているわけではありません．
各 tactic は，現在のゴールに対して，最終的には Lean のカーネルが検査できる証明項を構成します．
Infoview ではゴールが変形されて見えますが，背後では「どのコンストラクタ，どの関数，どの定理を使って項を作るか」を指定していると考えるとよいです．
...
| `P ∧ Q` を示す | `constructor`，`exact ⟨_, _⟩` | `And.intro` |
| `P ∧ Q` を使う | `cases`，`rcases`，`obtain` | `And.left`，`And.right`，パターン分解 |
| `P ↔ Q` を示す | `constructor` | `Iff.intro` |
| `P ↔ Q` を使う | `rw`，`.mp`，`.mpr` | 両方向の含意を field として使う |
| `P ∨ Q` を示す | `left`，`right` | `Or.inl`，`Or.inr` |
| `P ∨ Q` を使う | `cases`，`rcases` | `Or` のコンストラクタによる場合分け |
| `∃ x, P x` を示す | `use`，`exact ⟨x, hx⟩` | `Exists.intro` |
| `False` から任意の命題を示す | `exfalso`，`contradiction` | `False.elim` |
| `a = b` を示す | `rfl`，既存補題 | `Eq.refl` や `Eq` 型の証明 |
| `a = b` を使う | `rw`，`subst` | 等式による置換 |
| `inductive` 型を使う | `cases`，`induction` | コンストラクタによる場合分け・帰納法 |
| `structure` の等式を示す | `ext` | field ごとの等式 |

この対応を意識すると，tactic が失敗したときにも「いま必要なのは関数を作ることか，コンストラクタを使うことか，既存の等式で書き換えることか」を切り分けやすくなります．
以降の節では，この表の各行を具体例として見ていきます．
-/
...
/-
---
## `structure` と `inductive` 型に対応する tactic
...
/-
選言や `inductive` 型の値を分解するには `cases` を使います．
-/
...
/-
`structure` でも，同じ `structure` の値を field ごとに比べて等式を示すことがあります．
これは数学で「成分が等しいので，組や `structure` として等しい」と言う部分に対応します．
`@[ext]` を付けておくと，`ext` tactic が使う外延性補題が生成されます．
...
これらを使う場合も，Lean は最終的にその原理を含む証明項を構成し，カーネルが型を確認します．
...
`rw` と `simp` は書き換え，`intro` と `apply` は含意や全称命題，`constructor` と `cases` は論理結合子や `inductive` 型，`induction` は帰納法に対応します．
