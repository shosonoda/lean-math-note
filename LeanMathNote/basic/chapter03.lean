...
* `rw`，`simp`，`simpa`
* `calc` モード
* `constructor`，`obtain`，`rintro`，`left`，`right`，`match`，`cases`，`rcases`
* `induction`
* `congr`，`ext`，`funext`
...
| `P ∨ Q` を使う | `match`，`cases`，`rcases` | `Or` のコンストラクタによる場合分け |
...
| `inductive` 型を使う | `match`，`cases`，`induction` | コンストラクタによる場合分け・帰納法 |
