...
Lean では，定義につけた普通の名前だけでなく，数学に近い記号や専用の構文を使えます．
...
記法は読みやすさのための表面構文なので，分からない記法に出会ったら，まず `#check` で型を確認し，「これはどの定義の別表記か」を調べるのが有効です．
講義資料では記法を使いますが，必要に応じて元の定義名も併記します．
-/
...
ただし，読み手にとって由来が分かりにくくなることもあるので，講義資料では必要な範囲に限定して使うのがよいです．
-/
...
日本語で「クラス」と聞くと，オブジェクト指向プログラミングの class を連想するかもしれません．
Lean の型クラスはそれとは少し違います．
