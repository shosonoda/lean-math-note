...
* Mathematics in Lean, Sets and Functions: <https://leanprover-community.github.io/mathematics_in_lean/C04_Sets_and_Functions.html>
...
Lean は型理論を基礎にしているため，数学でいう「対象の集まり」はまず型として現れます．
...
この意味で，Lean の `Set α` は数学の「集合」そのものというより，固定された型 `α` 上の部分集合全体，つまり冪集合 `$\mathcal P(α)$` に対応します．
ただし実際の証明では，`Set` が関数として実装されていることを直接展開するより，`{x | p x}` や `x ∈ s` や `s ⊆ t` のような集合のインターフェースを使うのが普通です．

対応を大まかに書くと，次のようになります．

* 数学の全体集合・台集合: Lean では多くの場合 `α : Type*`
* 数学の部分集合 `s \subseteq α`: Lean では `s : Set α`
* 数学の冪集合 `$\mathcal P(α)$`: Lean では `Set α` という型
* 条件 `x \in s`: Lean では命題 `x ∈ s`

このため，`Set Nat` は「自然数という集合」ではなく，「自然数の部分集合を要素にもつ型」と読む方が正確です．
Mathlib のドキュメントでは `Set X` と呼ばれますが，直感的には `Subset X` と呼んだ方が意味が分かりやすい場面もあります．
-/
...
/-
数学では `Nat` や `ℝ` を「集合」と呼ぶことが多いですが，Lean の基礎ではそれらは型です．
これは単なる用語の違いではありません．
型理論では，型を ZFC の集合として素朴に解釈すると扱いにくい型も自然に現れます．
たとえば関数型や高階の型を含む定義では，ある型 `P` と，`P` から作った型 `T P` が同型になるような自己参照的な状況を考えられます．
擬似コードで書くと，次のような例です．
...
これは Lean の帰納型としてそのまま導入する例ではありませんが，型を集合として読むことの限界を示す標準的な警告として有用です．
もしそれらを素朴な集合と同一視し，濃度だけで

```text
|P| = |T P| = 2^(2^|P|)
```

のように読んでしまうと，Cantor 型の議論と衝突します．
Lean で日常的に数学を形式化するときにこの病理を直接扱うことは多くありませんが，
「Lean の `Type` は単に大きな集合論的宇宙の部分集合だ」と考えるのではなく，
「対象が住む型」と「その型の上の部分集合 `Set α`」を分けて読むことが重要です．

以後の章で位相空間の開集合，閉集合，コンパクト集合，零点集合などを書くときは，ほとんどの場合 `Set X` を使います．
代数の部分群や部分加群は，単なる `Set` ではなく，`Set` に閉性条件を束ねた構造体として現れます．
-/
